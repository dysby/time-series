...
fit <- y %>%
  model(
    sarima000011 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(2, 0, 0) + PDQ(0, 1, 2, period=24)),
  )
p1 <- fit %>% augment() %>% ACF(.innov, lag_max = 150) %>% autoplot() +
  ggtitle(paste(fit$Location, format(fit %>% pull(2)))) + ylim(-0.1,0.5)
p2 <- fit %>% augment() %>% PACF(.innov, lag_max = 150) %>% autoplot() + ylim(-0.1,0.5)
...
fit <- y %>%
  model(
    sarima000011 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(0, 0, 1) + PDQ(0, 1, 2, period=24))
  )
...
saveRDS(fit, file = "models/paio-pires_fit_s012.rds")
saveRDS(fit, file = "models/paio-pires_fit_s011.rds")
# Load the city object as city
fit <- readRDS("models/paio-pires_fit_s011.rds")
