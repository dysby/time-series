...
# sugere SARIMA (2,1,0)(0, 1, 1)
fit <- y %>%
  model(
    sarima000011 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(2, 1, 0) + PDQ(0, 1, 1, period=24)),
  )
p1 <- fit %>% augment() %>% ACF(.innov, lag_max = 150) %>% autoplot() + ylim(-0.1,0.5) + ggtitle(format(fit %>% pull(2)))
...
fit <- y %>%
  model(
    ...
    sarima400012 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(4, 0, 0) + PDQ(0, 1, 2, period=24)),
  )

m <- fit %>% select("Location", all_of("sarima200012"))
m <- fit %>% select("Location", all_of("sarima202012"))
p1 <- m %>% augment() %>% ACF(.innov, lag_max = 150) %>% autoplot() + ggtitle(format(m %>% pull(2))) + ylim(-0.5,1)
p2 <- m %>% augment() %>% PACF(.innov, lag_max = 150) %>% autoplot() + ylim(-0.5,1)
p1 + p2
...
# saveRDS(fit, file = "models/ilhavo_fit_s012.rds")
# Load the city object as city
fit <- readRDS("models/ilhavo_fit_s012.rds")

best_model_name = "sarima202012"
...
train_fit <- training %>%
  model(
    sarimafinal = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(2, 0, 0) + PDQ(0, 1, 2, period=24))
  )
