...
fit <- y %>%
  model(
    sarima000012 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(1, 0, 1) + PDQ(0, 1, 2, period=24)),
  )
p1 <- fit %>% augment() %>% ACF(.innov, lag_max = 150) %>% autoplot() +
...
fit <- y %>%
  model(
    sarima000011 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(2, 0, 1) + PDQ(0, 1, 2, period=24)),
  )
p1 <- fit %>% augment() %>% ACF(.innov, lag_max = 150) %>% autoplot() +
  ggtitle(paste(fit$Location, format(fit %>% pull(2)))) + ylim(-0.1,0.5)
p2 <- fit %>% augment() %>% PACF(.innov, lag_max = 150) %>% autoplot() + ylim(-0.1,0.5)
...
fit <- y %>%
  model(
    sarima000011 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(1, 0, 1) + PDQ(0, 1, 2, period=24)),
  )
p1 <- fit %>% augment() %>% ACF(.innov, lag_max = 150) %>% autoplot() +
  ggtitle(paste(fit$Location, format(fit %>% pull(2)))) +
  ylim(-0.1,0.5)
...
fit <- y %>%
  model(
    sarima000101 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(0, 0, 0) + PDQ(1, 0, 1, period=24)),
    sarima100101 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(1, 0, 0) + PDQ(1, 0, 1, period=24)),
    sarima001101 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(0, 0, 1) + PDQ(1, 0, 1, period=24)),
    ...
    sarima202101 = ARIMA(box_cox(Ozone, lambda) ~ 0 + pdq(2, 0, 2) + PDQ(1, 0, 1, period=24)),
...
saveRDS(fit, file = "models/entrecampos_fit_s011.rds")
# Load the city object as city
fit <- readRDS("models/entrecampos_fit_s012.rds")
